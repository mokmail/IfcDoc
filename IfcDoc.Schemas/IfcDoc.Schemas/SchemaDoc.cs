...
namespace IfcDoc.Schema.DOC
{
	...
	{ 
		...
		public void UpgradeExample(DocExample docExample, Dictionary<DocEntity, DocEntity> migration)
		{
			// load example
	...
	public class DocTemplateDefinition : DocObject // now inherits from DocObject
	{
		...
		public bool IsTemplateReferenced(DocTemplateDefinition docTemplateDefinition)
		{
		...
		ICloneable// abstract in IfcDoc 2.7
	{
		...
		public void setId(string uniqueStr)
		{
	...
	public abstract class DocOp : SEntity
	{
		...
		/// <param name="o"></param>
		/// <returns></returns>
		public virtual object Eval(object o, Hashtable population, DocTemplateDefinition template, Type valuetype, List<int> indexpath)
		{
			return null;
		}
	...
	public class DocSchema : DocObject
	{
		...
		public int UpdateDiagramPageNumbers()
		{
			...
			foreach (DocEntity docEnt in this.Entities)
			{
				...
				{
					// temp: force update
					int px = (int)(docEnt.DiagramRectangle.X * Factor / PageX);
					int py = (int)(docEnt.DiagramRectangle.Y * Factor / PageY);
			...
			foreach (DocType docType in this.Types)
			{
				...
				{
					// temp: force update
					int px = (int)(docType.DiagramRectangle.X * Factor / PageX);
					int py = (int)(docType.DiagramRectangle.Y * Factor / PageY);
					int page = 1 + py * this.DiagramPagesHorz + px;
			...
			if (this.SchemaRefs != null)
			{
				foreach (DocSchemaRef docRef in this.SchemaRefs)
				{
					foreach (DocDefinitionRef docDef in docRef.Definitions)
					{
						...
						{
							int px = (int)(docDef.DiagramRectangle.X * Factor / PageX);
							int py = (int)(docDef.DiagramRectangle.Y * Factor / PageY);
							int page = 1 + py * this.DiagramPagesHorz + px;
			...
			if (this.PageTargets != null)
			{
				foreach (DocPageTarget docPageTarget in this.PageTargets)
				{
					...
					{
						int px = (int)(docPageTarget.DiagramRectangle.X * Factor / PageX);
						int py = (int)(docPageTarget.DiagramRectangle.Y * Factor / PageY);
						int page = 1 + py * this.DiagramPagesHorz + px;
						docPageTarget.DiagramNumber = page;

						foreach (DocPageSource docPageSource in docPageTarget.Sources)
						{
							px = (int)(docPageSource.DiagramRectangle.X * Factor / PageX);
							py = (int)(docPageSource.DiagramRectangle.Y * Factor / PageY);
	...
	/// </summary>
		...
		public int GetDefinitionPageNumber(DocDefinition docEntity)
		{
			...
			int px = (int)(docEntity.DiagramRectangle.X * Factor / PageX);
			int py = (int)(docEntity.DiagramRectangle.Y * Factor / PageY);
			int page = 1 + py * this.DiagramPagesHorz + px;
			return page;
		}
		...
		IDocTreeHost
	{
		...
		public bool InheritanceDiagramFlag { get; set; }
		...
		// new version
		public DocAttribute ResolveAttribute(string attrname, DocProject docProject)
		{
		...
		public DocAttribute ResolveAttribute(string attrname, Dictionary<string, DocObject> map)
		{
			foreach (DocAttribute docAttr in this.Attributes)
			{
				if (docAttr.Name.Equals(attrname))
    ...
    */
	...
	public class DocPropertySet : DocVariableSet
	{
		...
		public DocProperty RegisterProperty(string p, DocProject project)
		{
		...
		public DocProperty GetProperty(string p)
		{
			foreach (DocProperty docProperty in this.Properties)
			{
				if (docProperty.Name.Equals(p))
	...
	public class DocProperty : DocObject
	{
		...
		public DocProperty RegisterProperty(string p)
		{
	...
	public class DocPropertyEnumeration : DocObject
	{
		...
		public DocPropertyConstant GetConstant(string p)
		{
	...
	public class DocQuantitySet : DocVariableSet
	{
		...
		public DocQuantity RegisterQuantity(string p, DocProject project)
		{
		...
		public DocQuantity GetQuantity(string p)
		{
			foreach (DocQuantity docQuantity in this.Quantities)
			{
				if (docQuantity.Name.Equals(p))
