	...
	public class SchemaSVG : IDisposable
	{
		...
		private DocRectangle LoadRectangle(g group)
		{
			...
			if (group.rect != null && group.rect.Count == 1)
			{
				...
				docRectangle.X = Double.Parse(r.x) / DocSchema.Factor;
				docRectangle.Y = Double.Parse(r.y) / DocSchema.Factor;
				docRectangle.Width = Double.Parse(r.width) / DocSchema.Factor;
				docRectangle.Height = Double.Parse(r.height) / DocSchema.Factor;
			}
		...
		private void LoadPoints(polyline p, List<DocPoint> listPoints)
		{
			...
			foreach (string point in points)
			{
				...
				docPoint.X = Double.Parse(coords[0]) / DocSchema.Factor;
				docPoint.Y = Double.Parse(coords[1]) / DocSchema.Factor;
						...
						foreach (g subgroup in group.groups)
						{
							foreach (DocAttribute docAttr in docEnt.Attributes)
							{
								if (docAttr.Name.Equals(subgroup.id))
								{
									...
									{
										double ax = Double.Parse(subgroup.text[0].x) / DocSchema.Factor;
										double ay = Double.Parse(subgroup.text[0].y) / DocSchema.Factor;
		...
		private g SaveDefinition(DocObject docObj, string displayname)
		{
			...
			double x = docDef.DiagramRectangle.X * DocSchema.Factor;
			double y = docDef.DiagramRectangle.Y * DocSchema.Factor;
			double cx = docDef.DiagramRectangle.Width * DocSchema.Factor;
			double cy = docDef.DiagramRectangle.Height * DocSchema.Factor;
			...
			if (this.m_format == DiagramFormat.UML)
			{
				...
				listPoint.Add(new DocPoint(x / DocSchema.Factor, y / DocSchema.Factor));
				listPoint.Add(new DocPoint((x + cx) / DocSchema.Factor, y / DocSchema.Factor));
					...
					if (docObjRef is DocEntity)
					{
						...
						foreach (DocAttributeRef docAttrRef in docDefRef.AttributeRefs)
						{
                            ...
                            if (this.m_format == DiagramFormat.UML)
                            {
								...
								double ty = docAttrRef.DiagramLine[0].Y * DocSchema.Factor;
								...
								text tr = new text();
								tr.x = (docAttrRef.DiagramLine[0].X * DocSchema.Factor + 4).ToString();
								tr.y = (ty).ToString();
		...
		private void SaveLine(g group, List<DocPoint> line, DocPoint prev, bool? bold)
		{
			...
			if (prev != null)
			{
				sb.Append(prev.X * DocSchema.Factor);
				sb.Append(",");
				sb.Append(prev.Y * DocSchema.Factor);
				sb.Append(" ");
			}

			foreach (DocPoint docLine in line)
			{
				sb.Append(docLine.X * DocSchema.Factor);
				sb.Append(",");
				sb.Append(docLine.Y * DocSchema.Factor);
				sb.Append(" ");
			}
		...
		public void Save()
		{
			svg s = new svg();
			s.version = "1.1";
			s.width = (this.m_schema.DiagramPagesHorz * DocSchema.PageX).ToString();
			s.height = (this.m_schema.DiagramPagesVert * DocSchema.PageY).ToString();
				...
				if (group != null)
				{
					...
					foreach (DocAttribute docAttr in docEnt.Attributes)
					{
						if (docAttr.DiagramLine != null && docAttr.DiagramLine.Count >= 2)
						{
							...
							t.fill = "black";
							t.x = (ax * DocSchema.Factor).ToString();
							t.y = (ay * DocSchema.Factor).ToString();
							t.alignment_baseline = "middle";
							t.text_anchor = "middle";
							t.font_size = "10";
							t.font_family = "Arial, Helvetica, sans-serif";
			...
			if (this.m_format == DiagramFormat.ExpressG)
			{
				...
				for (int iPageY = 0; iPageY < m_schema.DiagramPagesVert; iPageY++)
				{
					...
					{
						rect r = new rect();
						r.x = (iPageX * DocSchema.PageX).ToString();
						r.y = (iPageY * DocSchema.PageY).ToString();
						r.width = DocSchema.PageX.ToString();
						r.height = DocSchema.PageY.ToString();
						r.fill = "none";
						r.stroke = "lime";
						grid.rect.Add(r);
					}
