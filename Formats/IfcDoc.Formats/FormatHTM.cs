...
namespace IfcDoc.Format.HTM
{
		...
		IComparer<string>
	{
		...
		public void WriteTemplateTable(DocProject docProject, DocTemplateDefinition docTemplateDefinition, int level, Dictionary<DocObject, bool>[] dictionaryViews)
		{
			...
			string templateUrl = "../schema/templates/" + Compiler.MakeLinkName(docTemplateDefinition) + ".htm";
		...
		public void WriteInheritanceListing(string baseclass, bool predefined, string caption, DocModelView docView, string path, string filename, DocPublication docPublication)
		{
			...
			string linkid = "inheritance-" + Compiler.MakeLinkName(docView) + "-" + caption.ToLower();
			this.WriteLinkTo(docPublication, linkid, 3);
			this.WriteFooter(docPublication.Footer);

			using (FormatHTM htmLink = new FormatHTM(path + "/link/" + linkid + ".htm", this.m_mapEntity, this.m_mapSchema, this.m_included))
			{
				htmLink.WriteLinkPage("../annex/annex-c/" + Compiler.MakeLinkName(docView) + "/" + filename + ".htm", docPublication);
			}
		}
		...
		public void WriteDocumentationMarkup(string content, DocObject current, DocPublication docPublication, string htmlPath, string inputPath)
		{
			if (content == null)
				return;
		...
		public void WriteScriptToBlank(string path)
		{
			this.Write(
				"\r\n" +
		...
		public void WriteScript(int iSection, int iSchema, int iType, int iItem)
		{
			if (iSection < 0)
			{
				// annex
		...
		public void WriteContentRefs(List<ContentRef> listFigures, string prefix)
		{
			for (int iFigure = 0; iFigure < listFigures.Count; iFigure++)
			{
				...
				{
					link = "schema/templates/" + Compiler.MakeLinkName(target) + ".htm";
				}
				else if (target is DocEntity || target is DocType)
				{
					string schema = this.m_mapSchema[target.Name];
					link = "schema/" + schema.ToLower() + "/lexical/" + Compiler.MakeLinkName(target) + ".htm";
				}
				...
				{
					link = "annex/annex-e/" + Compiler.MakeLinkName(target) + ".htm";
				}
		...
		public static void WriteTOCforTemplates(List<DocTemplateDefinition> list, int level, string prefix, FormatHTM htmTOC, FormatHTM htmSectionTOC, Dictionary<DocObject, bool> included)
		{
			...
			foreach (DocTemplateDefinition docTemplateDef in list)
			{
				if (included == null || included.ContainsKey(docTemplateDef))
				{
					...
					string rellink = Compiler.MakeLinkName(docTemplateDef) + ".htm";
...
#region IComparer Members
		...
		public void WriteProperties(List<DocProperty> list, DocProject docProject, DocEntity docEntity, IList<string> locales)
		{
			if (list.Count == 0)
				return;
		...
		public void WriteLinkTo(DocProject docProject, DocPublication docPublication, DocObject type)
		{
			int levels = 3;
			...
			this.WriteLinkTo(docPublication, Compiler.MakeLinkName(type), levels);
		}

		public void WriteLinkTo(DocPublication docPublication, string identifier, int levels)
		{
			string up = "";
			for (int i = 0; i < levels; i++)
			{
		...
		public void WriteViewIcons(DocDefinition type, DocProject docProject, Dictionary<DocObject, bool>[] dictionaryViews, string path)
		{
			if (dictionaryViews != null && dictionaryViews.Length > 0)
			{
				for (int iMapView = 0; iMapView < dictionaryViews.Length; iMapView++)
				{
					...
					if (mapthis != null && mapthis.TryGetValue(type, out mapinclude) && mapinclude)
					{
						...
						{
							this.WriteLine("<a href=\"../../views/" + Compiler.MakeLinkName(docViewMap) + "/index.htm\" ><img src=\"../../../img/view-" + Compiler.MakeLinkName(docViewMap) + ".png\" title=\"" + docViewMap.Name + "\"/></a>");
						}
					}
				}
			}
		...
		public void WriteLinkPage(string linkurl, DocPublication docPublication)
		{
			this.WriteLine(
		...
		public void WriteComputerListing(string name, string code, int[] indexpath, DocPublication docPublication)
		{
			int iAnnex = -1;
		...
		{
			string defaultdesc = DocumentationHtmlNoParagraphs(entity.Documentation);
			...
			if (entity.Localization.Count > 0)
			{
				...
				{
					string localname = doclocal.Name;
					string localdesc = DocumentationHtmlNoParagraphs(doclocal.Documentation);

					string localid = doclocal.Locale.Substring(0, 2).ToLower();
					if (localid.Equals("en", StringComparison.InvariantCultureIgnoreCase) && string.IsNullOrEmpty(localdesc))
					{
						localdesc = DocumentationHtmlNoParagraphs(entity.Documentation);
						defaultdesc = DocumentationHtmlNoParagraphs(entity.Documentation);
					}
		...
		public void WriteEntityInheritance(DocEntity entity, DocEntity treeleaf, DocModelView[] views, Dictionary<DocObject, bool>[] viewmap, DocPublication docPublication, string htmlPath, ref int sequence, string inputPath)
		{
			if (entity.BaseDefinition != null)
			{
				if (this.m_mapEntity.ContainsKey(entity.BaseDefinition))
				{
					DocEntity baseEntity = (DocEntity)this.m_mapEntity[entity.BaseDefinition];
					WriteEntityInheritance(baseEntity, treeleaf, views, viewmap, docPublication, htmlPath, ref sequence, inputPath);
				}
			}
			...
			WriteEntityAttributes(entity, treeleaf, views, viewmap, docPublication, htmlPath, ref sequence, inputPath);
		}
		...
		public void WriteEntityAttributes(DocEntity entity, DocEntity treeleaf, DocModelView[] views, Dictionary<DocObject, bool>[] viewmap, DocPublication docPublication, string htmlPath, ref int sequence, string inputPath)
		{
			bool bInverse = false;
			bool bDerived = false;
			bool bExplicit = false;
				...
				foreach (DocAttribute attr in entity.Attributes)
				{
					...
					if (bInclude)
					{
						if (attr.Inverse == null && attr.Derived == null)
						{
							...
							if (this.m_included == null || this.m_included.ContainsKey(attr))
							{
								this.WriteDocumentationMarkup(DocumentationHtmlNoParagraphs(attr.Documentation), entity, docPublication, htmlPath, inputPath);
							}
							else
							{
		...
		public void WriteTerm(DocTerm docRef, int[] indexpath)
		{
			...
			this.WriteLine("<dt class=\"term\"><strong name=\"" + Compiler.MakeLinkName(docRef) + "\" id=\"" + Compiler.MakeLinkName(docRef) + "\">" +
				sbIndex.ToString() + " " + docRef.Name + "</strong></dt>");
			this.WriteLine("<dd class=\"term\">" + DocumentationHtml(docRef.Documentation));
		...
		public void WriteChangeLog(DocObject entity, List<DocChangeSet> listChangeSets, DocPublication docPublication)
		{
			...
			if (mapChange.Count > 0 || entity.IsDeprecated())
			{
				...
				{
					this.WriteLine("<td colspan=5><b><a href=\"../../../annex/annex-f/" + Compiler.MakeLinkName(docChangeSet) + "/index.htm\">" + docChangeSet.Name + "</a></b></td>");
		...
		public static string DocumentationHtmlNoParagraphs(string documentation)
		{
		...
		public static string DocumentationHtml(string documentation)
		{
