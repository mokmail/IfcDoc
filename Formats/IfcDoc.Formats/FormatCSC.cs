...
{
	public class FormatCSC : IDisposable,
		IFormatExtension
	{
		string m_filename;
					...
					foreach (DocSchema docSchema in docSection.Schemas)
					{
						...
						foreach (DocType docType in docSchema.Types)
						{
							...
							if ((options & DocCodeEnum.Documentation) != 0)
							{
								...
								using (StreamWriter writerHtml = new StreamWriter(path + @"\" + filehtml, false, Encoding.UTF8))
								{
									List<ContentRef> listFig = new List<ContentRef>();
									List<ContentRef> listTab = new List<ContentRef>();
									string doc = ContentRef.UpdateNumbering(docType.Documentation, listFig, listTab, docType);
